...
from pyteal import Bytes, compileTeal, Mode, Expr
...
def getAppTotalFunded(
    client: AlgodClient, appID: int
) -> Dict[bytes, Union[int, bytes]]:
    appInfo = client.application_info(appID)
    return appInfo["params"]["global-state"][2]["value"]["uint"]


def getUserInAppLocalState(client, addr):
    results = client.account_info(addr)
    local_state = results["apps-local-state"][0]
    return local_state


def getUserFundedAmount(client, addr):
    results = client.account_info(addr)
    return results["apps-local-state"][0]["key-value"][0].get("value").get("uint")
